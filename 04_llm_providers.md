...
graph TD
    subgraph core[ZeroClaw Core]
        ConfigHandler[("配置读取模块<br/>(src/config/schema.rs)")]
        ProviderRouter{"模型路由分发<br/>(src/providers/mod.rs)"}
        AgentEngine("Agent 决策核心")
    end

    subgraph sources[Config Sources]
        EnvVars["环境变量<br/>(*_API_KEY)"]
        TomlConfig["active_workspace.toml<br/>~/.zeroclaw/config.toml"]
    end
    ...
    subgraph providers[Plugin Providers]
        ...
        Compatible("兼容泛用 Provider<br/>vLLM/Ollama/...")
...
## 3.3 专项深挖：OpenAI OAuth 原生支持与高可用多号池 (Failover Pool) 
...
### 3.3.1 OAuth 授权与多号录入 (`src/oauth_cli.rs` & `src/oauth/openai_login.rs`)
...
### 3.3.2 凭据的多号池存储引擎 (`src/oauth/store.rs`)
...
### 3.3.3 Agent 运行时的全自动轮转与智能 Failover (`src/providers/openai_oauth.rs`)
...
sequenceDiagram
    ...
    loop Failover 重试循环 (Cursor Advance)
        ...
        alt 正常返回 (HTTP 200)
            ...
            Note over Provider: 触发 advance_cursor()：<br/>标记当前账号风控休眠，指针移向号池下一个有效账号
...
### 3.3.5 延伸对照：官方 OpenAI Codex CLI 的解法 (Keyring) 🔑
...
### 3.3.6 高阶技巧：拥抱开源生态 —— 将官方 Codex `auth.json` 零成本迁移至 ZeroClaw 🚀
