...
### 功能亮点：
1. **傻瓜式配置生成**：通过在终端里一问一答（比如：“你想使用哪个 LLM 供应商？”“你的 API Key 是什么？”“你要接入微信还是 Telegram？”），帮你自动在 `~/.zeroclaw/config.toml` 生成合格的配置。
...
2. **Channels**：拉起各种长连接 IM 平台监听器（比如扫码连着微信、Telegram Bot、Discord 的 WebSocket 连接）。
...
### 它承担的三大核心任务：
1. **接管所有远端 Webhook**：当你的 Telegram、QQ、企业微信收到消息时，腾讯/国外的服务器会把消息 POST 到你的机器上。Gateway 里的 `/qq`, `/telegram`, `/whatsapp` 等路由，就是专门张开嘴巴接收这些报文的地方。
...
> 
> 一旦底层的隧道打通，这不仅意味着你能接收微信消息，更意味着**彻底脱离了 IM 工具的束缚**。你的这台闲置 Mac 原地飞升，变成了一台暴露在公网上的“云服务器”：
...
3. **Daemon** 开始工作 ➡️ 它在后台拉起了微信监听，同时拉起并守护着 **Gateway**。
4. 别人给你发微信 ➡️ 微信服务器将消息投递到你公网的 **Gateway** 端口 ➡️ Gateway 将消息丢给 Agent 大脑处理。
