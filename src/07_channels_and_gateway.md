...
由于市面上有数百种 IM（即时通讯）和社群软件，它们的 API 千奇百怪。ZeroClaw 采用了适配器模式，用这一个 Trait 统御了所有的外部输入源（Telegram, Discord, Slack, 飞书/Lark, 钉钉等）：
...
3. **极强幂等墙 (IdempotencyStore)**:
   这对于即时通讯机器人开发**至关重要**。很多平台的 Webhook（特别是飞书和 WhatsApp）如果在 3 秒内未收到 200 OK，会疯狂开始重试。由于 Agent 大脑思考可能要十几秒，网关的 `IdempotencyStore` 会记录消息指纹，保证再多的重发砸过来也会被吃掉，保证 LLM 的 Token 配额**绝对不会被平行触发的幽灵请求双倍扣除**。
